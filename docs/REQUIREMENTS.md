...
BastionClaw gives you the core functionality without that mess.
...
### Message Routing
- A router listens to Telegram (and optionally WhatsApp) and routes messages based on configuration
...
- Bot token authentication via BotFather during setup

### Scheduler
- Built-in scheduler runs on the host, spawns containers for task execution
- Custom `bastionclaw` MCP server (inside container) provides scheduling tools
...
### Skills
- `/setup` - Install dependencies, configure Telegram bot, set up scheduler, start services
- `/customize` - General-purpose skill for adding capabilities (new channels like Slack, new integrations, behavior changes)
...
- **Main channel**: Personal DM with the bot (Telegram)

---

## Project Name

**BastionClaw** - A reference to Clawdbot (now OpenClaw).
