...
│  │    • mcp__bastionclaw__* (scheduler tools via IPC)                │   │
│  │                                                                │   │
│  └──────────────────────────────────────────────────────────────┘   │
│                                                                      │
└──────────────────────────────────────────────────────────────────────┘
...
│   ├── whatsapp-auth.ts           # Standalone WhatsApp authentication (optional)
...
│   ├── bastionclaw.log               # Host stdout
│   └── bastionclaw.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.bastionclaw.plist         # macOS service configuration
```
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'bastionclaw-agent:latest';
...
- `{{PROJECT_ROOT}}` - Absolute path to your bastionclaw installation
...
BastionClaw uses a hierarchical memory system based on CLAUDE.md files.
...
7. Router invokes Claude Agent SDK:
   ...
   └── mcpServers: bastionclaw (scheduler)
   │
   ▼
...
BastionClaw has a built-in scheduler that runs tasks as full agents in their group's context.
...
Claude: [calls mcp__bastionclaw__schedule_task]
        {
...
### BastionClaw MCP (built-in)

The `bastionclaw` MCP server is created dynamically per agent call with the current group's context.
...
BastionClaw runs as a single macOS launchd service.

### Startup Sequence

When BastionClaw starts, it:
1. **Ensures Apple Container system is running** - Automatically starts it if needed; kills orphaned BastionClaw containers from previous runs
2. Initializes the SQLite database (migrates from JSON files if they exist)
3. Loads state from SQLite (registered groups, sessions, router state)
4. Connects to Telegram (bot starts polling):
...
### Service: com.bastionclaw

**launchd/com.bastionclaw.plist:**
```xml
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/bastionclaw.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/bastionclaw.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.bastionclaw.plist ~/Library/LaunchAgents/
...
Inbound messages could contain malicious instructions attempting to manipulate Claude's behavior.
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep bastionclaw` |
| "Claude Code process exited with code 1" | Apple Container failed to start | Check logs; BastionClaw auto-starts container system but may fail |
| "Claude Code process exited with code 1" | Session mount path wrong | Ensure mount is to `/home/node/.claude/` not `/root/.claude/` |
| Session not continuing | Session ID not saved | Check SQLite: `sqlite3 store/messages.db "SELECT * FROM sessions"` |
| Session not continuing | Mount path mismatch | Container user is `node` with HOME=/home/node; sessions must be at `/home/node/.claude/` |
| Bot not responding | Bot token invalid or expired | Verify with `curl https://api.telegram.org/bot$TOKEN/getMe` |
| "No groups registered" | Haven't added groups | Use `@Andy add group "Name"` in main |

### Log Location

- `logs/bastionclaw.log` - stdout
- `logs/bastionclaw.error.log` - stderr
