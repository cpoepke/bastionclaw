...
- **Non-blocking Design** - Uses BastionClaw scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. BastionClaw already set up and running
...
Add Parallel AI MCP servers after the bastionclaw server:
```typescript
const mcpServers: Record<string, any> = {
  bastionclaw: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__bastionclaw__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__bastionclaw__send_message
   4. Use mcp__bastionclaw__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | container run -i --entrypoint /bin/echo bastionclaw-agent:latest "Container OK"
```
...
- Check task runs: `tail -f logs/bastionclaw.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.bastionclaw`
