...
description: Add voice message transcription to BastionClaw using OpenAI's Whisper API. Automatically transcribes WhatsApp voice notes so the agent can read and respond to them.
...
Then install it. **IMPORTANT:** The OpenAI SDK requires Zod v3 as an optional peer dependency, but BastionClaw uses Zod v4. This conflict is guaranteed, so always use `--legacy-peer-deps`:
...
Find the orphan cleanup block in `ensureContainerSystemRunning()` (the section starting with `// Kill and clean up orphaned BastionClaw containers from previous runs`) and replace it with:
...
```typescript
  ...
  try {
    ...
    const bastionclawContainers = containers.filter(
      (c) => c.configuration.id.startsWith('bastionclaw-'),
    );
    const running = bastionclawContainers
    ...
    const allNames = bastionclawContainers.map((c) => c.configuration.id);
...
nc = [c['configuration']['id'] for c in data if c['configuration']['id'].startswith('bastionclaw-')]
if nc: print(' '.join(nc))
...
```bash
launchctl kickstart -k gui/$(id -u)/com.bastionclaw
```

Verify it started with exactly one (or zero, before first message) bastionclaw container:

```bash
sleep 3 && launchctl list | grep bastionclaw
container ls -a --format json | python3 -c "
import sys, json
data = json.load(sys.stdin)
nc = [c for c in data if c['configuration']['id'].startswith('bastionclaw-')]
print(f'{len(nc)} bastionclaw container(s)')
...
```bash
tail -f logs/bastionclaw.log | grep -i "voice\|transcri"
```
...
```bash
# Stop all bastionclaw containers, then restart the service
container ls -a --format json | python3 -c "
import sys, json
data = json.load(sys.stdin)
running = [c['configuration']['id'] for c in data if c['configuration']['id'].startswith('bastionclaw-') and c['status'] == 'running']
...
```bash
tail -100 logs/bastionclaw.log | grep -i transcription
```
...
The OpenAI SDK requires Zod v3, but BastionClaw uses Zod v4. This conflict is guaranteed — always use:
```bash
npm install --legacy-peer-deps
```
