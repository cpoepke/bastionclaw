...
Yes. BastionClaw uses Docker, which works on both macOS and Linux. Just install Docker and run `/setup`.
```
...
```bash
echo '{}' | docker run -i --entrypoint /bin/echo bastionclaw-agent:latest "Container OK"
```
...
docker run --rm --entrypoint /bin/bash -v /tmp/test-ro:/test:ro bastionclaw-agent:latest \
...
```bash
mkdir -p /tmp/test-rw
docker run --rm --entrypoint /bin/bash -v /tmp/test-rw:/test bastionclaw-agent:latest \
