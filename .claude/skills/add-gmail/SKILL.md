...
>    - If prompted for consent screen, choose "External", fill in app name (e.g., "BastionClaw"), your email, and save
>    - For Application type, select **Desktop app**
>    - Name it anything (e.g., "BastionClaw Gmail")
>    - Click **Create**
...
```bash
sleep 2 && launchctl list | grep bastionclaw
```
...
export const EMAIL_CHANNEL: EmailChannelConfig = {
  ...
  triggerValue: 'BastionClaw',  // the label name, address pattern, or prefix
  contextMode: 'thread',
...
```bash
sleep 3 && tail -20 logs/bastionclaw.log | grep -i email
```
...
```bash
tail -f logs/bastionclaw.log | grep -E "(email|Email)"
```
