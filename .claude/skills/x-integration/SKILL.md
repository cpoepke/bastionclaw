---
name: x-integration
description: X (Twitter) integration for BastionClaw. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** BastionClaw v1.0.0. Directory structure may change in future versions.
...
1. **BastionClaw is installed and running** - WhatsApp connected, service active
...
# Verify: launchctl list | grep bastionclaw shows PID and exit code 0
```
...
| `BASTIONCLAW_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/bastionclaw.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into BastionClaw, make the following modifications:
...
All paths below are relative to project root (`BASTIONCLAW_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep bastionclaw  # Should show PID and exit code 0 or -
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/bastionclaw.log | tail -20

# Script errors
grep -i "error\|failed" logs/bastionclaw.log | tail -20
```
...
container run bastionclaw-agent ls -la /app/src/skills/
```
